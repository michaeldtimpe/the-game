...
Hit `GET /api/refresh` -- this re-scans all Plex libraries.
...
- `config.json` and `studio-lists/` are mounted as read-only volumes
- Synology Container Manager compatible (bridge network mode)
- Rebuild with `docker-compose up -d --build` after code changes
